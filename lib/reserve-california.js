...
var getSites = async function(campground, startDate, length) {
  ...
  const browser = await puppeteer.launch({headless: false}); // Launch visible for debugging
  const page = await browser.newPage();
  try {
    // Setup console logger inside page context. This is pretty spammy, so only turn on when debugging
    page.on('console', x => console.log(Date.now(), ">", x._text));
    ...
    // Need this for some reason. I would have expected the await for page.waitForNavigation to work
    spinWait(1);
